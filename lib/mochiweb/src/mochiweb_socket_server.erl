...
% ZVI : added - keepalive option
parse_options([{keepalive, KeepAlive} | Rest], State) ->
...
init(State=#mochiweb_socket_server{ip=Ip, port=Port, backlog=Backlog, nodelay=NoDelay, keepalive=KeepAlive}) -> % ZVI - added keepalive
    ...
    BaseOpts = [binary,
                ...
                {recbuf, ?RECBUF_SIZE},
                {active, false},
                {keepalive,KeepAlive}, % ZVI - added keepalive
                {nodelay, NoDelay}],
    Opts = case Ip of
        any ->
